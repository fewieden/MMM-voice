...
then
    ...
    then
        ...
        echo -e "\e[31m[STEP 2/8] Installing sphinxbase | Failed\e[0m"
        exit;
    fi
else
    cd sphinxbase
...
then
    ...
    then
        ...
        echo -e "\e[31m[STEP 3/8] Installing pocketsphinx | Failed\e[0m"
        exit;
    fi
else
    cd pocketsphinx
...
then
	echo -e "\e[31m[STEP 5/8] Installing cmuclmtk-0.7 | Failed\e[0m"
	exit;
fi
...
then
	echo -e "\e[31m[STEP 6/8] Installing tensorflow | Failed\e[0m"
	exit;
fi
...
then
    ...
    then
        ...
        echo -e "\e[31m[STEP 7/8] Installing g2p-seq2seq | Failed\e[0m"
        exit;
    fi
else
    cd g2p-seq2seq
